...
--					current_leq_tail: (1 < j and j < n - i + 2) implies ∀ t: (n - i + 2) |..| n ¦ a.sequence [j] <= a.sequence [t]
				...
				loop
					-- it2 := it2 + 1
					-- print ("inner iteration: " + it2.out + "%N")
					if a [j + 1] < a [j] then  -- a[2] < a [1]
							-- s.swap (j, j + 1, a)
						temp := a [j]
						a [j] := a [j + 1]
						a [j + 1] := temp
...
-- the program has to be verified before test generation
-- or at least its dependent classes/features need to be verified? -- limited to the classes that have no suppliers
