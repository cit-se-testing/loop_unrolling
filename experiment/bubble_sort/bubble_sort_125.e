...
feature
	...
	bubble_sort (a: SIMPLE_ARRAY [INTEGER]): detachable SIMPLE_ARRAY [INTEGER]
		note
			status: impure
			explicit: wrapping
		local
		...
		do
			...
			-- modify (a)
			-- sorted: attached Result as res and then ∀ l: 1 |..| res.sequence.count ¦ ∀ r: l |..| res.sequence.count ¦ res.sequence [l] <= res.sequence [r]
		end
