...
feature -- Basic operations
	...
	insertion_sort (a: SIMPLE_ARRAY [INTEGER])
		...
		require
			...
			a_size_limit: a.count <= 20 -- {INTEGER}.max_value
		...
		do
			...
			invariant
				a.is_wrapped
				-- i_in_range: 2 <= i and i <= a.count + 1

					-- is_permutation (a.sequence, a.sequence.old_)
				-- partially_sorted: is_part_sorted (a.sequence, 1, i - 1)
			until
				i > a.count or it1 > 20
			loop
				it1 := it1 + 1
				...
				invariant
					a.is_wrapped
					-- j_in_range: 1 <= j and j <= i

					partially_sorted_1: is_part_sorted (a.sequence, 1, j - 1)
					-- partially_sorted_2: is_part_sorted (a.sequence, j, i)
					-- is_sorted: across 1 |..| (j - 1) as k all across (j + 1) |..| i as l all a.sequence [k] <= a.sequence [l] end end
				until
					j = 1 or else a [j - 1] <= a [j] or it2 > 20
				loop
					it2 := it2 + 1
					swap (a, j, j - 1)
					j := j - 1
					i := i + 1 -- incorrect position of instruction
				-- variant
					-- j
				end
					-- i := i + 1
